...
function RequestDetail({ requestId, session, close }: { requestId: string; session: Session; close: () => void }) {
  const [request, setRequest] = useState<HelpRequest>()
  const [photoUrl, setPhotoUrl] = useState('')
  const [error, setError] = useState('')
  useEffect(() => {
    let active = true
    getPrivateRequest(session, requestId).then(async rows => {
      if (!active) return
      if (!rows[0]) { setError('No fue posible encontrar la solicitud.'); return }
      const privateRequest = mapPrivate(rows[0])
      setRequest(privateRequest)
      if (privateRequest.requestPhotoName) {
        try { setPhotoUrl(await getEvidenceUrl(session, privateRequest.requestPhotoName)) } catch { setPhotoUrl('') }
      }
    }).catch(reason => active && setError(reason instanceof Error ? reason.message : 'No fue posible cargar los detalles.'))
    return () => { active = false }
  }, [requestId, session])
  return <div className="modal-backdrop" onMouseDown={close}><section className="modal approval-detail request-detail" role="dialog" aria-modal="true" aria-labelledby="request-detail-title" onMouseDown={event => event.stopPropagation()}><button className="close" onClick={close}>×</button><span className="eyebrow">DETALLE COMPLETO · ADMINISTRACIÓN</span><h2 id="request-detail-title">{request?.publicCode ?? 'Solicitud'}</h2>{!request && !error && <p>Cargando información…</p>}{error && <p className="form-error">{error}</p>}{request && <><div className="detail-status"><span className={`request-status status-${request.status.toLowerCase().replace(' ', '-')}`}>{request.status}</span><b>{request.category}</b><span>{request.priority}</span></div><section className="detail-grid"><div><small>Nombre completo</small><b>{request.fullName}</b></div><div><small>Documento</small><b>{request.documentType} · {request.documentNumber}</b></div><div><small>Teléfono</small><a href={`tel:${request.phone}`}>{request.phone}</a></div><div><small>Fecha y hora</small><b>{requestDateTime(request.createdAt)}</b></div><div><small>Barrio</small><b>{request.neighborhood}</b></div><div><small>Dirección exacta</small><b>{request.address}</b></div><div><small>Latitud</small><b>{request.location?.latitude ?? 'No registrada'}</b></div><div><small>Longitud</small><b>{request.location?.longitude ?? 'No registrada'}</b></div><div className="wide"><small>Descripción de la ayuda</small><p>{request.description}</p></div></section>{photoUrl && <section className="detail-media single-media"><figure><figcaption>Fotografía de la solicitud</figcaption><img src={photoUrl} alt={`Fotografía de ${request.publicCode ?? 'la solicitud'}`} /></figure></section>}</>}</section></div>
}
...
export default function App() {
  const [view, setView] = useState<View>('dashboard'); const [requests, setRequests] = useState<HelpRequest[]>([]); const [completedMedia, setCompletedMedia] = useState<Record<string, { requestUrl?: string; solutionUrl?: string }>>({}); const [imagePreview, setImagePreview] = useState<{ url: string; alt: string }>(); const [centers, setCenters] = useState<CollectionCenter[]>([]); const [admins, setAdmins] = useState(initialAdmins); const [changes, setChanges] = useState(initialChanges); const [category, setCategory] = useState<'Todas' | Category>('Todas'); const [status, setStatus] = useState<'Todos' | Status>('Todos'); const [sort, setSort] = useState<Sort>('priority'); const [mapCategory, setMapCategory] = useState<'Todas' | Category>('Todas'); const [mapPriority, setMapPriority] = useState<'Todas' | Priority>('Todas'); const [page, setPage] = useState(1); const [showForm, setShowForm] = useState(false); const [changeFor, setChangeFor] = useState<HelpRequest>(); const [detailFor, setDetailFor] = useState<HelpRequest>(); const [showLogin, setShowLogin] = useState(false); const [mobileMenu, setMobileMenu] = useState(false); const [session, setSession] = useState<Session | null>(savedSession()); const [adminProfile, setAdminProfile] = useState<{ full_name: string; role: 'admin' | 'superadmin' } | null>(null); const [notice, setNotice] = useState('')
