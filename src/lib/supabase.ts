...
export async function uploadEvidence(folder: 'requests' | 'changes', file: File, session?: Session) {
  ...
  const response = await fetch(`${url}/storage/v1/object/request-evidence/${path}`, { method: 'POST', headers: { ...headers(session, false), 'Content-Type': file.type, 'x-upsert': 'false' }, body: file })
